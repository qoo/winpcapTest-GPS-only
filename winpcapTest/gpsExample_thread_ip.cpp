...
int main(int argc, char** argv)
{
	...
	/* User choose ip filtering for Lidar*/
	//char *filter = NULL;
	//string input = "";
	//while (true) {
	//	cout << "Please choose a Lidar to record (1: left, 2: right): ";
	//	getline(cin, input);
	//	if ( isdigit(input[0]) &&  input.substr(0, 1) == "1" ) {
	//		filter = "src 192.168.1.201"; // "host 151.101.45.105"
	//		break;
	//	}else if (isdigit(input[0]) && input.substr(0, 1) == "2") {
	...
	//filter = cinput;
	//cout << "Filtering ip: " << filter << endl;
	/* User choose port for GPS*/
	//string input = "";
	//while (true) {
	...
	// Lidar(argc, argv);
	//thread t2(&Lidar, argc, argv, filter);
